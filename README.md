...
To help you with the correct naming convention use the function from the
dataDocumentation package to create the meta data. Install the
dataDocumentation R package like this:
...
The function `create_durin_meta_data()` creates meta data. The meta data
can be made for the 4Corners, DroughtNet or Nutrient study. Use the
argument `study` to define which meta data you want. *4Corners*: study
on the 4 main Durin sites; DroughtNet: *droughtNet* experiment at Lygra
and Tjotta; *Nutrient*: nutrient experiment at Lygra
...
and you can choose a file name (not mandatory). The data will be stored
in the project directory. The file name always contains Durin and the
study, e.g. Durin_4Corners_filename.csv.
...
| File name | Naming convention | Example |
|:---|:---|:---|
| File name | Project_Status_Study_Approach_Response_Year(s).Extension | DURIN_clean_gradient_field_cflux_2023-2025.csv |
|  | Project | DURIN |
|  | Status | raw or clean |
|  | Study | *4Corners*: study on the 4 main Durin sites; *DroughtNet*: droughtNet experiment at Lygra and Tjotta (previously part of Landpress); *Gradient*: gradient study between 4 corners; *Nutrient*: nutrient experiment at Lygra; *ClimateChamber*: climate chamber experiment in Oslo; *LygraFire*: Fire related studies at Lygra |
